...
def list_tickets(enterprise_id: int = None, db: Session = Depends(get_db), _user=Depends(get_current_user)):
    if enterprise_id:
...
def create_ticket(ticket: schemas.SupportTicketBase, enterprise_id: int, db: Session = Depends(get_db), _user=Depends(get_current_user)):
...
def update_ticket_status(ticket_id: int, status_update: dict, db: Session = Depends(get_db), _user=Depends(get_current_user)):
