...
export default function Subscription() {
  ...
  const loadData = async () => {
    ...
    setLoading(true);
    try {
      const [entData, plansData] = await Promise.all([
        db.getEnterpriseDetail(enterpriseId),
        db.getPlans()
      ]);
      ...
      showToast('error', 'Failed to synchronize plan intelligence');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePlanSwitch = async (planId: string) => {
    if (!enterpriseId) return;
    try {
      await db.updateEnterprisePlan(enterpriseId, planId);
      showToast('success', `Successfully shifted instance to ${planId.toUpperCase()}`);
      loadData(); // Reload to show updated plan
    } catch (err: any) {
  ...
  const currentPlan = enterprise?.enterprise?.plan_id || enterprise?.plan_id; 
          ...
          <div className="lg:col-span-2 space-y-8">
                ...
                <div className="relative z-10">
                   <div className="flex justify-between items-start mb-6">
                      <div>
                         ...
                         <h2 className="text-4xl font-black tracking-tighter">{currentPlan?.toUpperCase() || 'STARTER'}</h2>
                      </div>
                      <div className="flex flex-col items-end">
                          ...
                          <p className="text-[10px] font-bold text-primary-200">UID: #{enterprise?.enterprise?.id || enterprise?.id}</p>
                      </div>
                   </div>
                   ...
                   <div className="grid grid-cols-2 gap-8 mb-8">
                      ...
                      <div>
                         ...
                         <p className="font-black text-2xl tracking-tight">Auto-Renewing</p>
                      </div>
                   </div>

                   <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      {enterprise?.plan?.features?.slice(0, 3).map((f: string, i: number) => (
                        <div key={i} className="flex items-center gap-3 bg-white/5 p-3 rounded-2xl border border-white/10">
                           <span className="material-symbols-outlined text-primary-300 text-sm">verified</span>
                           <span className="text-[10px] font-black uppercase tracking-widest truncate">{f}</span>
                        </div>
                      ))}
                      {!enterprise?.plan && (
                        <>
                          <div className="flex items-center gap-3 bg-white/5 p-3 rounded-2xl border border-white/10">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest">Core Access</span>
                          </div>
                          <div className="flex items-center gap-3 bg-white/5 p-3 rounded-2xl border border-white/10">
                            <span className="material-symbols-outlined text-primary-300">hub</span>
                            <span className="text-[10px] font-black uppercase tracking-widest">Standard Ops</span>
                          </div>
                        </>
                      )}
                   </div>
             ...
             <div className="bg-white dark:bg-[#1e293b] rounded-3xl border border-slate-200 dark:border-slate-700 p-8 shadow-sm">
                <div className="flex justify-between items-center mb-8">
                    ...
                    <button onClick={() => loadData()} className="text-[10px] font-black text-primary-600 bg-primary-50 dark:bg-primary-900/10 px-4 py-2 rounded-full uppercase tracking-widest hover:bg-primary-100">Refresh Pulse</button>
                </div>
                <div className="space-y-6">
                   <div>
                      <div className="flex justify-between text-[10px] font-black uppercase tracking-widest mb-2 text-slate-500">
                         ...
                         <span className="text-slate-900 dark:text-white">{enterprise?.stats?.staff_count || 1} / {currentPlan === 'PLAN-ENTERPRISE' ? '∞' : '10'}</span>
                      </div>
                   ...
                   <div>
                      <div className="flex justify-between text-[10px] font-black uppercase tracking-widest mb-2 text-slate-500">
                         ...
                         <span className="text-slate-900 dark:text-white">{enterprise?.stats?.total_jobs || 0} Jobs Logged</span>
                      </div>
                      <div className="h-2.5 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden shadow-inner">
          ...
          <div className="space-y-8">
             <div className="bg-white dark:bg-[#1e293b] rounded-3xl border border-slate-200 dark:border-slate-700 p-8 shadow-sm">
                ...
                <div className="space-y-4">
                  {plans.map(plan => (
                    <div key={plan.id} className={`p-6 border-2 rounded-3xl relative overflow-hidden transition-all ${currentPlan === plan.id ? 'border-primary-500 bg-primary-50/30 dark:bg-primary-500/5' : 'border-slate-100 dark:border-slate-800 hover:border-primary-200'}`}>
                       <div className="flex justify-between items-center mb-4">
                          <h4 className={`font-black text-sm uppercase tracking-widest ${currentPlan === plan.id ? 'text-primary-600 dark:text-primary-400' : 'text-slate-500'}`}>{plan.name}</h4>
                          {currentPlan === plan.id && (
                       ...
                       {currentPlan !== plan.id ? (
                         <button 
                           ...
                           className="w-full py-4 bg-slate-900 hover:bg-black text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl transition-all active:scale-[0.98]"
                         >
                           Switch Instance
                         </button>
                       ) : (
                         <div className="w-full py-4 bg-primary-100 dark:bg-primary-900/30 text-primary-600 rounded-2xl font-black text-[10px] uppercase tracking-widest text-center">
                            Currently In Use
                         </div>
                       )}
                    </div>
