...
def list_appointments(enterprise_id: int, db: Session = Depends(get_db)):
    return db.query(models.Appointment).filter(models.Appointment.enterprise_id == enterprise_id).all()

@router.post("/", response_model=schemas.Appointment)
def create_appointment(appointment: schemas.AppointmentBase, enterprise_id: int, db: Session = Depends(get_db)):
...
def update_appointment_status(app_id: int, status_update: dict, db: Session = Depends(get_db)):
