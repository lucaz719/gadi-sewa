...
def create_inventory_item(item: schemas.InventoryItemBase, db: Session = Depends(get_db), _user=Depends(require_role("garage", "vendor", "admin"))):
...
def get_inventory(skip: int = 0, limit: int = 100, db: Session = Depends(get_db), _user=Depends(require_role("garage", "vendor", "admin"))):
    return db.query(models.InventoryItem).offset(skip).limit(limit).all()

@router.delete("/{item_id}")
def delete_item(item_id: int, db: Session = Depends(get_db), _user=Depends(require_role("garage", "vendor", "admin"))):
...
def update_stock(update: StockUpdate, db: Session = Depends(get_db), _user=Depends(require_role("garage", "vendor", "admin"))):
