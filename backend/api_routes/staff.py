...
def list_staff(enterprise_id: int, db: Session = Depends(get_db), _user=Depends(get_current_user)):
...
def create_staff(staff: schemas.StaffMemberBase, enterprise_id: int, db: Session = Depends(get_db), _user=Depends(get_current_user)):
...
def update_staff(staff_id: int, staff_update: dict, db: Session = Depends(get_db), _user=Depends(get_current_user)):
...
def delete_staff(staff_id: int, db: Session = Depends(get_db), _user=Depends(get_current_user)):
    db_staff = db.query(models.StaffMember).filter(models.StaffMember.id == staff_id).first()
    if not db_staff:
        raise HTTPException(status_code=404, detail="Staff not found")
    db.delete(db_staff)
